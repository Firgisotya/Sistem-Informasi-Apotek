...
error_reporting(0);
if (isset($_GET['kd_persediaan'])) {
	$kd = $_GET['kd_persediaan'];
} else {
	echo "<script>alert('Kode Persediaan Belum Dipilih');document.location='manajemen-persediaan.php'</script>";
}
$data = mysqli_query($conn,"SELECT * FROM persediaan where kd_persediaan='$kd'")or die(mysqli_error());
...
<head>
    ...
    <title>Apochecker | Edit Persediaan</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Edit Persediaan</h1>
                    </div>
                            ...
                            <div class="card-body">
                                <form action="" method="post" enctype="multipart/form-data">
                                    <div class="form-group">
                                        <input type="hidden" name="kd_persediaan"
                                            value="<?php echo $daftar['kd_persediaan']; ?>">
                                        <label>Kode Persediaan</label>
                                        <input type="text" class="form-control" id="kd_persediaan" placeholder=""
                                            required="" name="kd_persediaan"
                                            value="<?php echo $daftar['kd_persediaan'];?>" readonly="">
                                    </div>
                                    <div class="form-group">
                                        <label>Bulan</label>
...
if (isset($_POST['edit'])) {
	...
	$kd = $_POST['kd_persediaan'];
    $ktgr = $_POST['jenis_obat'];
	$bln = $_POST['bulan'];
	$thn = $_POST['tahun'];
	$jml = $_POST['jumlah_obat'];

		$edit = mysqli_query($conn,"UPDATE persediaan SET bulan='$bln',jenis_obat='$ktgr',tahun='$thn', jumlah_obat='$jml' where kd_persediaan='$kd'");
		if($edit){
			echo "<script>alert('Data Persediaan berhasil diedit'); document.location='manajemen-persediaan.php'</script>";
		} else {
			echo "<script>alert('Data Persediaan gagal diedit')</script>";
